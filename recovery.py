...
from equationsOfMotionTools import euler, RK4


def loadModel(modelName):
    modelToReturn = pickle.load(open('models/'+str(modelName)+'_model.pickle','rb'))
    modelToReturn.lambdified = [sympy.lambdify(modelToReturn.symbols,equation) for equation in modelToReturn.equations]
...
class diffEqLinearModel():
    def __init__(self, ID = 'untitledModel'):
        ...
        self.longForm = None
        self.equations = None #shortform equations
        self.dt = 1
        self.error = None
        self.entropy = None
    ...
    def info(self):
        ...
        print(self.basis)
        print()
        print('Short-Form Diff Equations: ')
        for i in range(len(self.equations)):
            print(str(self.symbols[i])+'_{t+1} = ' + str(self.equations[i]))
        print()
        print('Model Entropy: ')
        print(self.entropy)
    ...
    def save(self,filename = None):
        ...
        if filename is None:
            ...
            pickle.dump(self, open('models/' + str(filename) + '_model.pickle', 'wb'))
    ...
    def fastFillFromDataset(self,reconstructedDataSet,basisType,order,printouts = False,shortformThresh = .01):
        ...
        self.equations = self.AijShort.dot(self.basis)
        self.longFormDiffEq = self.getDiffEqFromMap(self.Aij)
        self.shortFormDiffEq = self.getDiffEqFromMap(self.AijShort)
        #self.lambdified = [lambdify([tuple(self.symbols)],equationboi,'numpy') for equationboi in self.AijShort.dot(self.basis)]
        self.lambdified = [lambdify(tuple(self.symbols), equationboi, 'numpy') for equationboi in self.AijShort.dot(self.basis)]
    ...
    def leastSquaresAij(self,reconstructedDataSet,printouts):
        ...
        #Getting out the appropriate values
        for i in range(len(allAij)):
            for j in range(len(allAij[i])):
                if str(self.symbols[i]) == str(self.basis[j]):
                    allAij[i,j] = allAij[i,j]-1
        #scaling by appropriate dt
        allAij = allAij/self.dt
    ...
    def evolve(self,nPoints,initialConditions,integrationMethod = RK4):
        allPoints = [initialConditions]
        for i in range(nPoints):
            allPoints.append(self.predict(allPoints[-1],integMethod = integrationMethod))
        return allPoints

    def predict(self,point,tauFuture=1,integMethod = RK4):
        currentPrediction = point
        for i in range(tauFuture):
            currentPrediction = integMethod(self.lambdified,currentPrediction,self.dt)
...
class mapLinearModel():
    def __init__(self, ID = 'untitledModel'):
        ...
        self.componentErrors = []
        self.longForm = None
        self.equations = None #short form
        self.error = None
        self.entropy = None
        self.lambdified = None
